...
from monai.transforms import (
    ...
    Resized, RandFlipd, RandAffined, EnsureTyped
)
...
# Config (all overridable via environment variables, so a SLURM script can run different ablation arms without editing this file each time)
...
train_transforms = Compose([
    ...
    MultiWindowIntensityd(keys=["image"]),  # -> 3-channel: lung / soft-tissue / bone
...
print(f"Building 3D ResNet50 (multi-window, 3-channel input) on: {device}")

model = resnet50(
    spatial_dims=3,
    n_input_channels=3,   # <-- only architectural change vs. Model 2 (was 1)
    num_classes=18
).to(device)
...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "resnet50_multiwindow_best.pth")

    print("Training Complete. Model saved to resnet50_multiwindow_best.pth")
